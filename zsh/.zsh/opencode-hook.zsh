...
# ~/.zsh/utils/hooks.zsh
#
# Shell hook functions for directory changes and special operations
#
# =============================================================================

# Launch OpenCode with Ollama server and local models
opencode() {
    # check for Opencode and Ollama
    if ! command -v opencode &>/dev/null; then
       echo "Opencode not installed"
        ...
        if [[ ! ( -z "$selection" || "$selection" == "$no_model" ) && "$selection" != "$OLLAMA_LOADED_MODEL" ]]; then
            ...
            kill $(ps | grep ollama | awk '{print $1}' | head -n 1) 2>/dev/null
        fi
