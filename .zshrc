...
if [[ "$OS_FLAG" == "macos" ]]; then
    ...
    # ============================================
    if [[ -d "/opt/ros" ]]; then
        # Find available ROS distributions
        local ros_distros=($(ls -1d /opt/ros/*/ 2>/dev/null | xargs -n1 basename | sort -r))

        if [[ ${#ros_distros[@]} -gt 0 ]]; then
            # Use the first (latest) distro found
            local target_distro="${ROS_DISTRO:-${ros_distros[1]}}"
            local setup_file="/opt/ros/${target_distro}/setup.sh"
