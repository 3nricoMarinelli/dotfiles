...
# Global variable to track dotfiles source for cleanup
DOTFILES_SOURCE_GLOBAL=""
...
install_dotfiles() {
  print_step "Moving custom dotfiles to home directory..."

  # Determine source directory
  # Priority: 1) Script's directory, 2) ~/dotfiles, 3) Clone from remote
  SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  DOTFILES_SOURCE=""

  if [ -f "$SCRIPT_DIR/.zshrc" ] && [ -d "$SCRIPT_DIR/.config" ]; then
    DOTFILES_SOURCE="$SCRIPT_DIR"
    print_step "Using dotfiles from script directory: $SCRIPT_DIR"
  elif [ -f "$HOME/dotfiles/.zshrc" ] && [ -d "$HOME/dotfiles/.config" ]; then
    DOTFILES_SOURCE="$HOME/dotfiles"
    print_step "Using dotfiles from ~/dotfiles"
  else
    print_step "Cloning dotfiles from GitHub..."
    DOTFILES_SOURCE=$(mktemp -d)
    git clone --depth 1 https://github.com/3nricoMarinelli/dotfiles "$DOTFILES_SOURCE"
  fi

  # Store for cleanup at the end
  DOTFILES_SOURCE_GLOBAL="$DOTFILES_SOURCE"

  print_step "Moving dotfiles to home directory..."

  # Use nullglob to handle empty glob patterns
  shopt -s nullglob
  for item in "$DOTFILES_SOURCE"/.[!.]* "$DOTFILES_SOURCE"/*; do
    # Get just the filename (not the full path)
    filename=$(basename "$item")

    # Skip if item doesn't exist (includes broken symlinks)
    if [ ! -e "$item" ]; then
      continue
    fi

    # Skip git directory, scripts, docs, system-specific configs, runtime files
    case "$filename" in
    .git | .gitignore | .bash_history | .zsh_history | .cache | .local | setup.sh | fresh-terminal.sh | README.md | .p10k-linux.zsh | cleanup_ros_px4.sh | *.bak)
      continue
      ;;
    *)
      # Check if file/directory already exists in home directory
      if [ -e "$HOME/$filename" ]; then
        # Remove old backup if it exists, then create new backup
        if [ -e "$HOME/$filename.bak" ]; then
          rm -rf "$HOME/$filename.bak"
        fi
        mv "$HOME/$filename" "$HOME/$filename.bak"
        print_warning "Backed up existing $filename to $filename.bak"
      fi

      if mv "$item" "$HOME/" 2>/dev/null; then
        print_success "Moved $filename"
      else
        print_warning "Failed to move $filename (skipping)"
      fi
      ;;
    esac
  done
  ...
  print_success "Dotfiles moved to home directory"
}
...
main() {
  ...
  # Phase 4: Terminal multiplexer
  install_tmux
  install_tpm

  # Phase 5: Text editor (install before dotfiles to ensure nvim config works)
  install_neovim

  # Phase 6: Move custom configurations
  install_dotfiles

  # Phase 7: Set default shell
  change_shell

  # Phase 8: Cleanup - remove dotfiles directory
  if [ -n "$DOTFILES_SOURCE_GLOBAL" ] && [ -d "$DOTFILES_SOURCE_GLOBAL" ]; then
    print_step "Removing dotfiles directory..."
    rm -rf "$DOTFILES_SOURCE_GLOBAL"
